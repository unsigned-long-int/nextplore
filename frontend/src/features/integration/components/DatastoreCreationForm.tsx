...
import { useCreateDatastore } from '@/features/integration/hooks/useCreateDatastore.ts';
import { DatastoreForm } from '@/features/integration/components/DatastoreForm.tsx';
import { DB, type DataStoreCreateRequest } from '@/shared/api/services/integration/types.gen';
...
export const DatastoreCreationForm = () => {
    const theme = useMantineTheme();
    const createDatastore = useCreateDatastore();
    const [modalOpened, setModalOpened] = useState(false);
    const [selectedDb, setSelectedDb] = useState<DB | null>(null);
    const openModalFor = (datastoreKey: string) => {
        setSelectedDb(datastoreKey as DB);
        setModalOpened(true);
    };

    const handleFormSubmit = async (data: DataStoreCreateRequest) => {
        try {
            await createDatastore.mutateAsync(data);
            showNotification({
    ...
    return (
        <>
            <Menu transitionProps={{ transition: 'pop-top-right'}} position='top-end' width={220} withinPortal radius='md'>
                ...
                <Menu.Dropdown>
                    {DATASTORES.map(({ key, label, icon, shortcut }) => (
                        <Menu.Item
                            key={key}
            ...
            <Modal
                ...
                title={`Create ${selectedDb?.toUpperCase() ?? ''} Data Store`}
                size='lg'
            >
                {selectedDb && (
                    <DatastoreForm
