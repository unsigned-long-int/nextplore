...
import { useTestDatastore } from '@/features/integration/hooks/useTestDatastore.ts';
import { useGetDescriptionEnhancement } from "@/features/ai-query/hooks/useGetDescriptionEnhancement.ts";
import { DatastoreCloudProviderPicker, type CloudProvider } from '@/features/integration/components/DatastoreCloudProviderPicker.tsx';
import { CertPicker } from '@/features/cert/components/CertPicker';
import { Auth, DB, Cloud, type DataStoreCreateRequest } from '@/shared/api/services/integration/types.gen';
...
type Props = {
    service_type: DB;
    onSubmit: (values: DataStoreCreateRequest) => void;
};

export const DatastoreForm: React.FC<Props> = ({ service_type, onSubmit }) => {
    ...
    const form = useForm<DataStoreCreateRequest>({
        initialValues: {
    ...
    const handleSubmit = (values: DataStoreCreateRequest) => {
        onSubmit(form.getTransformedValues(values));
    };

    const handleTest = async (value: DataStoreCreateRequest) => {
        const payload = form.getTransformedValues(value);
        try {
            const result = await testDatastore.mutateAsync(payload);
                ...
                <Stack gap='md'>
                    ...
                    <Group justify='flex-end' mt='sm' gap='sm'>
                        <Button
                            ...
                            loading={testDatastore.isPending}
                        >
                            Test Connection
                        </Button>
                        <Button
