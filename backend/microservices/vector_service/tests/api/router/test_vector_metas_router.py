...
class TestGetVectorMetas(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('api.router.get_vector_metas_router.vector_service_cache')
    ...
    async def test_cache_hit_returns_cached_directly(
        ...
        mock_cache.get_vector_metas = AsyncMock(return_value=cached_response)
        mock_cache.set_vector_metas = AsyncMock()

        connector = MagicMock(name='DatabaseBackendConnector')

        result = await get_vector_metas(payload, connector)

        self.assertIs(result, cached_response)

        mock_cache.get_vector_metas.assert_awaited_once_with(
            user_identity=user_identity, request=payload
        )

        mock_repo_cls.assert_not_called()
        mock_resp_cls.assert_not_called()
        mock_cache.set_vector_metas.assert_not_awaited()
    ...
    async def test_cache_miss_queries_repo_builds_response_and_sets_cache(
        ...
        mock_cache.get_vector_metas = AsyncMock(return_value=None)
        mock_cache.set_vector_metas = AsyncMock()
        ...
        mock_cache.set_vector_metas.assert_awaited_once_with(
