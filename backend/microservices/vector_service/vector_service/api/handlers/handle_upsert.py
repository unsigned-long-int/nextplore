...
async def handle_vector_upsert(
    ...
    try:
        ...
        logger.error(
            f"Upsert vector metadata failed with DB error: {e!s}",
            exc_info=True,
        ...
        logger.error(
            f"Upsert vector to vector DB failed with client error: {e!s}",
            exc_info=True,
            extra={"org_id": event.organization_id, "user_id": event.user_id},
        )
        raise
        ...
        logger.error(
            f"Unexpected error by handling upsert vectors: {e!s}",
            exc_info=True,
            extra={"org_id": event.organization_id, "user_id": event.user_id},
        )
        raise
