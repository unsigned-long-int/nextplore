...
class VectorRepository:
    ...
    async def get_profiles(
        ...
        try:
            ...
            msg = f"Get vector profiles failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise VectorProfilesGetFailed(msg) from e

    async def get_vectors(
        self, organization_id: UUID, user_id: UUID, vector_ids: list[UUID]
    ) -> list[Row]:
        try:
            if not vector_ids:
                logger.warning(
            ...
            msg = f"Get vectors failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def get_vector_count(self, organization_id: UUID, user_id: UUID) -> int:
        try:
            ...
            msg = f"Get vector count failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise VectorCountGetFailed(msg) from e

    async def upsert_vector_meta(
        self, organization_id: UUID, user_id: UUID, vectors_orm: list[VectorORM]
    ) -> None:
        try:
            async with self._db.session_scope(
                organization_id, user_id
            ...
            msg = f"Upsert vectors failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise VectorUpsertFailed(msg) from e

    async def delete_vector_meta(
        self, organization_id: UUID, user_id: UUID, datastore_id: UUID
    ) -> None:
        try:
            async with self._db.session_scope(
    ...
    async def get_qdrant_vector_ids(
        self, organization_id: UUID, user_id: UUID, datastore_id: UUID
    ) -> list[UUID]:
