...
def refine_filters_from_dto(
    lookup_query: SemanticCacheLookupQuery,
) -> list[FieldCondition]:
    filters = []
    if lookup_query.provider:
        filters.append(
            FieldCondition(
                key="provider", match=MatchValue(value=lookup_query.provider)
            )
        )
    if lookup_query.model_id:
        filters.append(
            FieldCondition(
                key="model_id", match=MatchValue(value=lookup_query.model_id)
            )
        )
    if lookup_query.model_ref_id is not None:
        filters.append(
            FieldCondition(
                key="model_ref_id",
                match=MatchValue(value=str(lookup_query.model_ref_id)),
            )
        )
    return filters
