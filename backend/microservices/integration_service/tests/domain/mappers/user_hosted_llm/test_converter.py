...
class TestOrmFromUserLlm(unittest.TestCase):
    ...
    def test_maps_ciphertext(self):
        self.assertEqual(
            self._call().encrypted_connection_params,
            self.llm.encrypted_conn_params.ciphertext,
        )
    ...
    def test_maps_wrapped_dek(self):
        self.assertEqual(
            self._call().wrapped_dek, self.llm.encrypted_conn_params.wrapped_dek
        )
...
class TestUserLlmFromOrm(unittest.TestCase):
    ...
    def test_builds_encrypted_secret_with_ciphertext(self):
        self.assertEqual(
            self._call().encrypted_conn_params.ciphertext,
            self.orm.encrypted_connection_params,
        )

    def test_builds_encrypted_secret_with_wrapped_dek(self):
        self.assertEqual(
            self._call().encrypted_conn_params.wrapped_dek, self.orm.wrapped_dek
        )

    def test_aad_contains_organization_id(self):
        self.assertEqual(
            self._call().encrypted_conn_params.aad["organization_id"],
            self.organization_id,
        )

    def test_aad_contains_user_id(self):
        self.assertEqual(
            self._call().encrypted_conn_params.aad["user_id"], self.user_id
        )

    def test_aad_contains_api_base(self):
        self.assertEqual(
            self._call().encrypted_conn_params.aad["api_base"], self.orm.api_base
        )

    def test_aad_contains_model_id(self):
        self.assertEqual(
            self._call().encrypted_conn_params.aad["model_id"], self.orm.model_id
        )
...
class TestUserLlmFromDto(unittest.TestCase):
    ...
    def _call(self, **overrides) -> UserLlm:
        with patch(
            f"{MODULE}.encrypt_conn_params", return_value=self.mock_encrypted_secret
        ) as self.mock_encrypt:
...
class TestUserLlmProfileFromOrm(unittest.TestCase):
    def test_returns_user_llm_profile_instance(self):
        self.assertIsInstance(
            user_llm_profile_from_orm(make_user_llm_orm()), UserLlmProfile
        )
    ...
    def test_maps_custom_api_base(self):
        result = user_llm_profile_from_orm(
            make_user_llm_orm(api_base="https://custom.endpoint.com/v1")
        )
        self.assertEqual(result.api_base, "https://custom.endpoint.com/v1")

    def test_maps_custom_model_id(self):
        result = user_llm_profile_from_orm(
            make_user_llm_orm(model_id="claude-sonnet-4-6")
        )
