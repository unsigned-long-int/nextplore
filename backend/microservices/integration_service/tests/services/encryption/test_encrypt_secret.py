...
from integration_service.services.encryption import encrypt_conn_params, encrypt_secret
...
class TestEncryptSecret(unittest.TestCase):
    ...
    def test_encrypt_secret_with_additional_kwargs(self):
        ...
        with patch(
            "integration_service.services.encryption.encrypt_secret.DataStoreSecret"
    ...
    def test_encrypt_secret_crypto_client_exception_propagates(self):
        self.mock_crypto_client.encrypt_secret.side_effect = Exception(
            "Encryption failed"
        )
...
class TestEncryptConnParams(unittest.TestCase):
    ...
    def test_aad_contains_exactly_four_keys(self):
        ...
        aad = self.mock_crypto_client.encrypt_secret.call_args.kwargs["aad"]
        self.assertSetEqual(
            set(aad.keys()), {"organization_id", "user_id", "api_base", "model_id"}
        )
    ...
    def test_crypto_client_error_propagates(self):
        self.mock_crypto_client.encrypt_secret.side_effect = RuntimeError(
            "vault unreachable"
        )
