...
class TestInspectTables(unittest.TestCase):
    ...
    def test_calls_inspector_methods_with_correct_parameters(self):
        ...
        self.mock_inspector.get_columns.assert_called_once_with(
            table_name=table_name, schema=self.schema_name
        )
...
class TestInspectSchemas(unittest.TestCase):
    ...
    @patch(
        "integration_service.services.crawl.catalog_builder.inspectors.HasSelectPermissionSpec"
    )
    @patch(
        "integration_service.services.crawl.catalog_builder.inspectors.inspect_tables"
    )
    ...
    def test_skips_schemas_not_satisfying_spec(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
        mock_perm_spec_cls.return_value = self._make_permission_spec(is_empty=False)
        self.mock_inspector.get_schema_names.return_value = [
            "schema1",
        ...
        self.mock_schema_spec.is_satisfied_by.side_effect = lambda c: (
            c.name
            in [
                "schema1",
                "schema3",
    ...
    def test_skips_schemas_with_empty_permissions(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
        self.mock_inspector.get_schema_names.return_value = [
            "schema1",
        ...
        def perm_spec_side_effect(crawler, schema_name):
            return self._make_permission_spec(
                is_empty=(schema_name == "performance_schema")
            )
    ...
    def test_inspect_tables_not_called_for_empty_permission_schemas(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
    ...
    def test_skips_schemas_with_no_tables(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
        mock_perm_spec_cls.return_value = self._make_permission_spec(is_empty=False)
        self.mock_inspector.get_schema_names.return_value = [
        ...
        mock_inspect_tables.side_effect = (
            lambda crawler, datastore_id, schema_name, table_spec: (
    ...
    def test_passes_combined_spec_to_inspect_tables(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
    ...
    def test_permission_spec_created_per_schema(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
        mock_perm_spec_cls.return_value = self._make_permission_spec(is_empty=False)
        self.mock_inspector.get_schema_names.return_value = [
    ...
    def test_creates_schema_catalog_with_correct_attributes(
        self, mock_inspect_tables, mock_perm_spec_cls
    ):
        mock_perm_spec_cls.return_value = self._make_permission_spec(is_empty=False)
...
class TestInspectorsDataStore(unittest.TestCase):
    ...
    def test_full_workflow(self, mock_inspect_tables, mock_perm_spec_cls):
        ...
        private_tables = (TableCatalog(datastore_id=datastore_id, name="credentials"),)
