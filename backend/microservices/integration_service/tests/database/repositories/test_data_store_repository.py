...
from svc_integration_contracts.models import DB, Auth, CertState, Cloud
...
class TestDataStoreRepository(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.repository = DataStoreRepository(
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.datastore_from_orm"
    )
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.orm_from_datastore_create"
    )
    async def test_create_datastore_returns_datastore_id(
        self, orm_from_datastore_create_mock
    ):
    ...
    async def test_create_datastore_raises_exception_on_database_error(
        self, orm_from_datastore_create_mock
    ):
        datastore_create = DataStoreCreate(
            auth=Auth.iam,
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.datastore_profile_from_orm"
    )
    async def test_get_datastore_profiles_returns_list(
        self, datastore_profile_from_orm_mock
    ):
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.orm_from_secrets"
    )
    async def test_create_secrets_succeeds(self, orm_from_secrets_mock):
        ...
        await self.repository.create_secrets(
            organization_id=self.organization_id, user_id=self.user_id, secrets=secrets
        )
    ...
    async def test_create_secrets_raises_exception_on_database_error(
        self, orm_from_secrets_mock
    ):
        secrets = {
            SecretType.PASSWORD: DataStoreSecret(
                organization_id=self.organization_id,
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.secrets_from_orm"
    )
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.orm_from_cert"
    )
    ...
    async def test_create_cert_raises_exception_on_database_error(
        self, orm_from_cert_mock
    ):
        cert = Cert(
            cert_kid="cert-kid-123",
            cert_name="test-cert",
    ...
    @patch(
        "integration_service.database.repositories.data_store_repository.cert_profile_from_orm"
    )
    async def test_get_datastore_cert_profiles_returns_list(
        self, cert_profile_from_orm_mock
    ):
    ...
    async def test_update_datastore_raises_exception_when_not_found(self):
        ...
        self.session_mock.execute.side_effect = [lock_result_mock, update_result_mock]
    ...
    async def test_update_datastore_increments_version_correctly(
        self, orm_from_secrets_mock
    ):
        datastore_update = DataStoreUpdate(
            connection_name="updated-connection",
