...
class TestCertProfilesRouter(unittest.TestCase):
    ...
    def test_returns_cached_cert_profiles(self, get_current_identity_mock):
        ...
        self.assertEqual(
            response.json(), [item.model_dump(mode="json") for item in cached]
        )
        self.cache_mock.get_datastore_cert_profiles.assert_awaited_once_with(
    ...
    def test_requests_cert_profiles_and_sets_cache(
        ...
        integration_repo_instance.get_datastore_cert_profiles.return_value = [
            self.domain_profile
        ]
        ...
        integration_repo_cls_mock.assert_called_once_with(
            self.database_backend_connector_mock
        )
    ...
    @patch("integration_service.api.router.cert_profiles_router.DataStoreRepository")
    @patch("integration_service.api.router.cert_profiles_router.get_current_identity")
    def test_db_error_returns_424(
        self, get_current_identity_mock, integration_repo_cls_mock
    ):
        user_identity_mock = MagicMock()
        user_identity_mock.user_id = uuid4()
        user_identity_mock.organization_id = uuid4()
        ...
        self.cache_mock.get_datastore_cert_profiles.return_value = None
        integration_repo_instance = AsyncMock()
        integration_repo_instance.get_datastore_cert_profiles.side_effect = (
            CertGetFailed("boom")
        )
        integration_repo_cls_mock.return_value = integration_repo_instance
