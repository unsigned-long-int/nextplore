...
class TestTestRouter(unittest.TestCase):
    ...
    def test_tests_postgres_datastore_successfully(
        ...
        payload["aws_external_id"] = (
            self.postgres_request.aws_external_id.get_secret_value()
            if self.postgres_request.aws_external_id
            else None
        )
        payload["aws_role_arn"] = (
            self.postgres_request.aws_role_arn.get_secret_value()
            if self.postgres_request.aws_role_arn
            else None
        )
        payload["password"] = (
            self.postgres_request.password.get_secret_value()
            if self.postgres_request.password
            else None
        )
        payload["client_secret"] = (
            self.postgres_request.client_secret.get_secret_value()
            if self.postgres_request.client_secret
            else None
        )
        payload["username"] = (
            self.postgres_request.username.get_secret_value()
            if self.postgres_request.username
            else None
        )
        ...
        self.engine_manager_mock.acquire_engine.assert_awaited_once_with(
            profile_instance
        )
    ...
    def test_tests_snowflake_datastore_with_certificate(
        ...
        response = self.client.post(
            self._url(), json=self.snowflake_request.model_dump(mode="json")
        )
    ...
    def test_tests_aws_datastore_with_role(
        ...
        payload["password"] = (
            self.aws_request.password.get_secret_value()
            if self.aws_request.password
            else None
        )
        payload["client_secret"] = (
            self.aws_request.client_secret.get_secret_value()
            if self.aws_request.client_secret
            else None
        )
        ...
        self.assertEqual(
            call_kwargs["aws_role_arn"], "arn:aws:iam::123456789012:role/MyRole"
        )
    ...
    def test_raises_exception_when_connection_fails(
        ...
        response = self.client.post(
            self._url(), json=self.postgres_request.model_dump(mode="json")
        )
    ...
    def test_handles_optional_fields_as_none(
        ...
        response = self.client.post(
            self._url(), json=minimal_request.model_dump(mode="json")
        )
