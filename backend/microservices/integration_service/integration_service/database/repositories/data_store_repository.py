...
from integration_service.database.models import CertORM, DataStoreORM, SecretORM
from integration_service.domain.mappers.cert import cert_profile_from_orm, orm_from_cert
...
class DataStoreRepository:
    ...
    async def get_user_datastore_ids(
        ...
        try:
            ...
            msg = f"Get datastore IDs failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise DataStoreGetFailed(msg) from e

    async def get_datastore(
        self, user_id: UUID, organization_id: UUID, datastore_id: UUID
    ) -> DataStore:
        try:
            async with self._db.session_scope(
                organization_id, user_id
                ...
                if datastore is None:
                    raise DataStoreNotFound(
                        f"No datastore found for ID: {datastore_id}"
                    )
                return datastore_from_orm(datastore)
        except SQLAlchemyError as e:
            msg = f"Get datastore {datastore_id} failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise DataStoreGetFailed(msg) from e
    ...
    async def get_datastore_by_id(
        ...
        try:
            async with self._db.session_scope(
                ...
                result = await scoped_session.execute(
                    select(DataStoreORM).where(DataStoreORM.id == datastore_id)
                )
                datastore = result.scalar_one_or_none()
                if datastore is None:
                    raise DataStoreNotFound(
            ...
            msg = f"Get datastore by ID failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise DataStoreGetFailed(msg) from e

    async def create_datastore(
        self, organization_id: UUID, user_id: UUID, datastore_create: DataStoreCreate
    ) -> UUID:
        try:
            ...
            msg = f"Create datastore failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def delete_datastore(
        ...
        try:
            async with self._db.session_scope(
                ...
                stmt = delete(DataStoreORM).where(
            ...
            msg = f"Delete data store failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def get_datastore_profiles(
        ...
        try:
            async with self._db.session_scope(
                ...
                return [
                    datastore_profile_from_orm(datastore) for datastore in datastores
                ]
        except SQLAlchemyError as e:
            msg = f"Get data store profiles failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def create_secrets(
        ...
        try:
            ...
            msg = f"Create secrets failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def get_secrets(
        ...
        try:
            ...
            msg = f"Get secrets failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise SecretsGetFailed(msg) from e

    async def get_kek_kid(
        self, datastore_id: UUID, user_id: UUID, organization_id: UUID
    ) -> str:
        try:
            async with self._db.session_scope(
                organization_id, user_id
            ...
            msg = f"Get kek_kid failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise KekKidGetFailed(msg) from e

    async def create_cert(
        self, organization_id: UUID, user_id: UUID, cert: Cert
    ) -> None:
        try:
            ...
            msg = f"Create certificate failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def get_datastore_cert_profiles(
        ...
        try:
            ...
            msg = f"Get certificate profiles failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
    ...
    async def update_datastore(
        ...
        try:
            async with self._db.session_scope(
                ...
                latest_version = await self._get_latest_secrets_version(
                    active_session, datastore_id
                )
                new_version = latest_version + 1
                await self._insert_secrets(active_session, secrets, new_version)
                await active_session.flush()
        except SQLAlchemyError as e:
            msg = f"Update data store failed with database error: {e!s}"
            logger.error(msg, exc_info=True)
            raise DataStoreUpdateFailed(msg) from e

    async def _get_latest_secrets_version(
        self, active_session: AsyncSession, datastore_id: UUID
    ) -> int:
        result = await active_session.execute(
            select(func.coalesce(func.max(SecretORM.version), 0)).where(
                SecretORM.datastore_id == datastore_id
            )
        )
        current_version = result.scalar_one()
        return current_version

    async def _lock_datastore(
        self, active_session: AsyncSession, datastore_id: UUID
    ) -> None:
        u = datastore_id.int
