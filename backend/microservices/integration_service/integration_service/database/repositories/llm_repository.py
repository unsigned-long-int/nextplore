...
from integration_service.domain.mappers.user_llm.converter import orm_from_user_llm, user_llm_profile_from_orm
    ...
    async def get_user_llm_profiles(
        ...
        try:
            async with self._db.session_scope(organization_id, user_id) as scoped_session:
                ...
                user_llms = result.scalars().all()
                return [user_llm_profile_from_orm(llm) for llm in user_llms]
        except SQLAlchemyError as e:
