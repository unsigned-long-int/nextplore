    ...
    for datastore_id in datastore_ids:
        try:
            ...
            if not datastore_spec.is_satisfied_by(datastore_meta_candidate):
                logger.info(f'Data store {datastore_id} is not satisfied by spec. Skipping.')
                continue
            ...
            logger.info(f'Crawled datastore {datastore_id} in {elapsed:.2f} seconds (schemas={len(schemas)})')
            if schemas:
                datastores.append(DataStoreCatalog(id=datastore_id, schemas=schemas))
        except ConnectionFailed as e:
            logger.warning(f'Data store {datastore_id} Connection failed: {str(e)}')
        except OperationalError as e:
            logger.warning(f'Data store {datastore_id} SQL operation failed: {str(e)}')
        except Exception as e:
            logger.exception(f'Data store {datastore_id} Unexpected error: {type(e).__name__}: {e}')
