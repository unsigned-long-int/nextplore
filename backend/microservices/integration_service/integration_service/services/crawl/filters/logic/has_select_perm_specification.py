...
from sqlalchemy import literal, select, table, text
...
class HasSelectPermissionSpec(Specification):
    ...
    def _fetch_accessible(
        self, crawler: Inspector, schema_name: str
    ) -> set[str] | None:
        dialect = crawler.dialect.name
        logger.info(
            "Fetching accessible permissions for schema %s, dialog: %s",
            schema_name,
            dialect,
        )
        try:
