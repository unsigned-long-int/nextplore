...
@router.get(
    "/organizations/{organization_id}/users/{user_id}/datastores/stats",
    response_model=DataStoreStatsResponse,
)
async def get_datastore_stats(
    ...
    if (
        ...
        or organization_id != user_identity.organization_id
    ):
        logger.error(
    ...
    try:
        cached = await cache_service.get_datastore_stats(user_identity=user_identity)
        if cached:
            return cached
        ...
        response = DataStoreStatsResponse(
            datastore_ids=datastore_ids, datastore_count=len(datastore_ids)
        )
        ...
        logger.error(
            f"Get data store stats request failed with DB error: {e}", exc_info=True
        )
        raise HTTPException(
        ...
        logger.error(
            f"Get data store stats failed with unexpected error: {e}", exc_info=True
        )
        raise HTTPException(
