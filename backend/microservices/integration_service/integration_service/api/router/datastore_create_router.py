...
async def create_datastore(
    ...
    try:
        ...
        logger.error(f"Create data store failed with DB error: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
            detail={"message": f"Database error: {e!s}"},
        )
    except Exception as e:
        logger.error(f"Unexpected create data store error: {e!s}.", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"message": f"Unexpected error while creating data store: {e!s}"},
        )
