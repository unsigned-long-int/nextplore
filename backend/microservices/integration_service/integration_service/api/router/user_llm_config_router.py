...
async def get_user_llm_configs(
    ...
    try:
        user_llm_config = await llm_service.get_user_llm_config(
            user_identity=user_identity, model_id=model_id
        )
        return user_llm_config
    except UserLlmGetFailed as e:
        logger.error(
            f"Get user llm config request failed with DB error: {e}", exc_info=True
        )
        raise HTTPException(
        ...
        logger.error(
            f"Get user llm config failed with unexpected error: {e!s}", exc_info=True
        )
        raise HTTPException(
