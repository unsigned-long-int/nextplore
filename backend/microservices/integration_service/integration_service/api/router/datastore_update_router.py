...
async def update_datastore(
    ...
    try:
        await datastore_service.update_datastore(
            user_identity=user_identity, datastore_id=datastore_id, payload=payload
        )
    except (DataStoreUpdateFailed, KekKidGetFailed) as e:
        logger.error(f"Update data store failed with DB error: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
            detail={"message": f"Database error: {e!s}"},
        )
    except Exception as e:
        logger.error(f"Unexpected update data_store error: {e!s}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"message": f"Unexpected error while updating data_store: {e!s}"},
        )
