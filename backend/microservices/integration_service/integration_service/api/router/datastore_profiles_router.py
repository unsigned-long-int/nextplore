...
async def get_datastore_profiles(
    ...
    try:
        cached = await cache_service.get_datastore_profiles(
        ...
        logger.error(
            f'Get data store profiles request failed with DB error: {e}',
            exc_info=True
        )
        raise HTTPException(
        ...
        logger.error(
            f'Get data store profiles failed with unexpected error: {str(e)}',
            exc_info=True
        )
        raise HTTPException(
