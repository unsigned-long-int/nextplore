...
async def test_datastore(
    ...
    try:
        connection_profile = ConnectionProfile(
            ...
            aws_external_id=payload.aws_external_id.get_secret_value()
            if payload.aws_external_id
            else None,
            aws_role_arn=payload.aws_role_arn.get_secret_value()
            if payload.aws_role_arn
            else None,
            snowflake_private_key=payload.snowflake_private_key.get_secret_value()
            if payload.snowflake_private_key
            else None,
        ...
        logger.error(f"Test data store failed with DB error: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_424_FAILED_DEPENDENCY,
            detail={"message": f"Database error: {e!s}"},
        )
    except Exception as e:
        logger.error(f"Unexpected test data store error: {e}", exc_info=True)
        raise HTTPException(
