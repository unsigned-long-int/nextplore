...
class TestHFProvider(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.inference_provider.get_model_response = AsyncMock()
    ...
    async def test_successfully_retrieves_model_response(self):
        ...
        self.inference_provider.get_model_response.return_value = valid_response
        provider = HFProvider(
            model=self.model,
        ...
        self.inference_provider.get_model_response.assert_awaited_once_with(
            hf_path=hf_path,
    ...
    async def test_raises_by_invalid_response(self):
        ...
        self.model.max_tokens = max_tokens
        self.inference_provider.get_model_response.return_value = invalid_response
        provider = HFProvider(
            model=self.model,
            inference_provider=self.inference_provider
        )
