...
@router.post(
    "/organizations/{organization_id}/users/{user_id}/llm/test",
    status_code=status.HTTP_204_NO_CONTENT,
)
async def test_user_llm(
    organization_id: UUID, user_id: UUID, payload: UserLlmTestRequest
) -> None:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={"message": f"Llm response error: {e!s}"},
        )
    except Exception as e:
        logger.error(
            f"Unexpected llm test error: {e!s}.",
            extra={
                "organization_id": user_identity.organization_id,
                "user_id": user_identity.user_id,
                "model_id": payload.model_id,
