...
class TestOrmContextRouter(unittest.TestCase):
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_returns_200_on_cache_miss(
        self, mock_identity, mock_resolve, mock_dispatch, mock_adapt
    ):
        mock_identity.return_value = self.mock_identity
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_executes_structured_query_with_payload(
        self, mock_identity, mock_resolve, mock_dispatch, mock_adapt
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
        ...
        self.mock_provider.execute_structured_query.assert_awaited_once_with(
            self.request
        )
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_adapts_raw_model_response(
        self, mock_identity, mock_resolve, mock_dispatch, mock_adapt
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_sets_cache_with_correct_args(
        self, mock_identity, mock_resolve, mock_dispatch, mock_adapt
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
