...
class TestGetExpandedQuery(unittest.TestCase):
    ...
    def test_returns_200_with_multi_query_response(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_original_query_is_available(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_variants_capped_at_multiplier(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_resolves_provider_params_with_correct_args(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_dispatches_provider_with_resolved_params(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_expands_query_with_correct_args(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_executes_expanded_query_via_provider(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    def test_returns_cached_response_when_available(
        ...
        mock_resolve.return_value = self.mock_provider_params
        mock_dispatch.return_value = self.mock_provider_factory
        cached = MultiQueryResponse(
            original_query="Show me all Klingon characters", variants=["List Klingons"]
        )
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_does_not_call_provider_on_cache_hit(
        self, mock_identity, mock_resolve, mock_dispatch
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
        mock_dispatch.return_value = self.mock_provider_factory
        self.cache_mock.get_expanded_query = AsyncMock(
            return_value=MultiQueryResponse(
                original_query="cached_query", variants=["cached query"]
            )
        )
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_does_not_resolve_params_on_cache_hit(
        self, mock_identity, mock_resolve, mock_dispatch
    ):
        mock_identity.return_value = self.mock_identity
        self.cache_mock.get_expanded_query = AsyncMock(
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_stores_response_in_cache_on_cache_miss(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_returns_424_when_invalid_model_response(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
        ...
        self.assertEqual(424, response.status_code)
        self.assertIn(
            "model returned unexpected format", response.json()["detail"]["message"]
        )
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_does_not_cache_on_provider_error(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
    ...
    @patch(f"{MODULE}.resolve_llm_provider_params")
    @patch(f"{MODULE}.get_current_identity")
    def test_does_not_cache_on_unexpected_error(
        self, mock_identity, mock_resolve, mock_dispatch, mock_expand
    ):
        mock_identity.return_value = self.mock_identity
        mock_resolve.return_value = self.mock_provider_params
        mock_dispatch.return_value = self.mock_provider_factory
        self.mock_provider.execute_query = AsyncMock(side_effect=RuntimeError("boom"))
