...
class TestUserLlmTestRouter(unittest.TestCase):
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_calls_prompt_model_with_hi_and_max_tokens_1(
        self, mock_from_dto, mock_dispatch, mock_identity
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_converts_payload_to_params(
        self, mock_from_dto, mock_dispatch, mock_identity
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_dispatches_with_resolved_params(
        self, mock_from_dto, mock_dispatch, mock_identity
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_returns_424_when_empty_response(
        self, mock_from_dto, mock_dispatch, mock_identity
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_returns_424_when_none_response(
        self, mock_from_dto, mock_dispatch, mock_identity
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_returns_424_when_invalid_model_response_raised(
        self, mock_from_dto, mock_dispatch, mock_identity
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
    ...
    def test_logs_unexpected_error_with_exc_info(
        self, mock_from_dto, mock_dispatch, mock_identity, mock_logger
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
        provider = AsyncMock()
    ...
    @patch(f"{MODULE}.dispatch_provider_factory")
    @patch(f"{MODULE}.user_llm_params_from_dto")
    def test_logs_unexpected_error_context(
        self, mock_from_dto, mock_dispatch, mock_identity, mock_logger
    ):
        mock_identity.return_value = self.user_identity
        mock_from_dto.return_value = self.mock_params
