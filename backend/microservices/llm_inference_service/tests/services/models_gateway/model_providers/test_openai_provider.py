...
from llm_inference_service.services.models_gateway.model_providers.openai_provider import (
    OpenAiProvider,
)
...
class TestOpenAiProviderBaseKwargs(unittest.TestCase):
    ...
    def test_exact_keys(self):
        provider = OpenAiProvider("gpt-4o")
        self.assertSetEqual(set(provider.base_kwargs().keys()), {"model", "api_key"})
...
class TestOpenAiProviderMaxTokens(unittest.TestCase):
    def _make_model_info(self, max_tokens: int | None) -> MagicMock:
        info = MagicMock()
        info.get.side_effect = lambda key, *args: (
            max_tokens if key == "max_tokens" else None
        )
        return info

    def test_returns_value_from_litellm(self):
        with patch(
            "llm_inference_service.services.models_gateway.model_providers.openai_provider.litellm"
        ) as mock_litellm:
