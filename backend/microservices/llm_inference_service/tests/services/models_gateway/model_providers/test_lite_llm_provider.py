...
from svc_llm_inference_contracts.models import LlmOutputSpecs, ORMContextRequest
...
class TestPromptModel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_passes_prompt_as_user_message(self):
        ...
        self.assertEqual(
            call_kwargs["messages"], [{"role": "user", "content": "my prompt"}]
        )
...
class TestExecuteStructuredQuery(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.mock_completion = AsyncMock(
            return_value=make_tool_call_response(self.raw_args)
        )
        self.provider = ConcreteLiteLlmProvider(completion_fn=self.mock_completion)
    ...
    async def test_passes_query_as_user_message(self):
        with (
            ...
            await self.provider.execute_structured_query(
                make_orm_request(query="show me revenue")
            )
    ...
    async def test_passes_tool_choice_required(self):
        with (
            ...
            self.assertEqual(
                self.mock_completion.call_args.kwargs["tool_choice"], "required"
            )
