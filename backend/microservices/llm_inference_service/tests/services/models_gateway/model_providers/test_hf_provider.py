...
from llm_inference_service.services.models_gateway.model_providers.hf_provider import (
    HFProvider,
)
...
class TestHFProviderModelPath(unittest.TestCase):
    def test_model_path_prefixes_with_openai(self):
        ...
        self.assertEqual(
            provider.model_path(), "openai/meta-llama/Llama-3.1-8B-Instruct"
        )
...
class TestHFProviderBaseKwargs(unittest.TestCase):
    ...
    def test_base_kwargs_contains_api_base(self):
        ...
        self.assertEqual(
            provider.base_kwargs()["api_base"], "https://router.huggingface.co/v1"
        )
    ...
    def test_base_kwargs_keys(self):
        provider = HFProvider(make_model())
        self.assertSetEqual(
            set(provider.base_kwargs().keys()), {"model", "api_key", "api_base"}
        )
