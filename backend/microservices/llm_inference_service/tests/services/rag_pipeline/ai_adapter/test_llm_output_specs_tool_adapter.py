...
QUALIFIED_COLUMNS_EMP  = ['hr.employees.employee_id', 'hr.employees.full_name', 'hr.employees.department_id']
...
class TestBuildToolSchemaQualifiedColumns(unittest.TestCase):
    ...
    def test_single_table_produces_correct_qualified_columns(self):
        props = build_tool_schema(make_single_table_specs())[0]['function']['parameters']['properties']
        self.assertEqual(props['column_names']['items']['enum'], ['hr.employees.employee_id'])
...
class TestBuildToolSchemaFilters(unittest.TestCase):
    ...
    def setUp(self):
        self.specs = make_llm_output_specs()
        self.filter_schema = build_tool_schema(self.specs)[0]['function']['parameters']['properties']['column_filters']
    ...
    def test_filter_required_fields(self):
        self.assertEqual(set(self.filter_items['required']), {'operator', 'value', 'filter_column'})
    ...
    def test_operator_enum_matches_filter_op_enum(self):
        self.assertEqual(self.filter_items['properties']['operator']['enum'], self.specs.filter_op_enum)
    ...
    def test_value_accepts_number_and_string(self):
        self.assertEqual(self.filter_items['properties']['value']['type'], ['number', 'string'])
...
class TestBuildToolSchemaAggregates(unittest.TestCase):
    ...
    def setUp(self):
        self.specs = make_llm_output_specs()
        self.agg_schema = build_tool_schema(self.specs)[0]['function']['parameters']['properties']['column_aggregates']
    ...
    def test_agg_func_enum_matches_agg_funcs_enum(self):
        self.assertEqual(self.agg_items['properties']['agg_func']['enum'], self.specs.agg_funcs_enum)
