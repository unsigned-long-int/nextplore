...
class TestCacheService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_embedding(self, get_cache_key_mock):
        ...
        self.cache.get_one.assert_awaited_once_with(
            self.organization_id, self.user_id, "key123", model=EmbeddingResponse
        )

    @patch("embedding_service.cache.cache_service.get_cache_key", return_value="key123")
    async def test_set_embedding(self, get_cache_key_mock):
        ...
        self.cache.set_one.assert_called_once_with(
            self.organization_id, self.user_id, "key123", value=response
        )
