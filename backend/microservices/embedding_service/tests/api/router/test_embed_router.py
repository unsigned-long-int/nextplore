...
class TestEmbed(unittest.TestCase):
    def setUp(self):
        ...
        self.app.dependency_overrides = {get_cache_service: lambda: self.cache_mock}
    ...
    @patch(
        "embedding_service.api.router.embed_router.get_current_identity",
        return_value="no-matter",
    )
    def test_returns_cached_embedding(self, get_current_identity_mock):
        ...
        response = self.client.post(
            "/v1/embedding/embed", json=self.request.model_dump()
        )
    ...
    def test_builds_embedding_and_adds_cache(
        self, get_current_identity_mock, dispatch_embedder_mock
    ):
        ...
        self.embedder_instance_mock.generate_embedding.assert_awaited_once_with(
            self.request.datastream
        )
    ...
    @patch("embedding_service.api.router.embed_router.dispatch_embedder")
    def test_raises_missing_embedder_engine(
        self, dispatch_embedder_mock, get_current_identity_mock
    ):
        self.cache_mock.get_embedding.return_value = None
