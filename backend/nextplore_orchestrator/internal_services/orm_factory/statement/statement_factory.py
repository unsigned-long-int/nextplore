...
def aggregate(statement_request: StatementRequest) -> Select:
    aggregate_columns = [aggregator['agg_column'] for aggregator in statement_request.column_aggregates]
    columns = [getattr(statement_request.orm_model, column) for column in statement_request.column_names if column not in aggregate_columns]
    for agg in statement_request.column_aggregates:
        ...
        aggregate_expression = AGGREGATOR_DISPATCHER[agg_func](getattr(statement_request.orm_model, agg_column)).label(f'{agg_func}_{agg_column}')
        columns.append(aggregate_expression)
    stmt = select(*columns)
    stmt = stmt.group_by(*[getattr(statement_request.orm_model, column) 
