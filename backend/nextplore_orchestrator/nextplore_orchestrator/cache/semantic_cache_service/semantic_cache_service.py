...
from svc_nextplore_orchestrator_contracts.models import AIQueryResponse, AIQueryRequest
from svc_vector_contracts.models import SemanticCacheLookupQuery, SemanticCacheEntry
...
class SemanticCacheService:
    ...
    async def lookup_semantic_cache(
        ...
        try:
            ...
            logger.error(f'Semantic cache VECTOR STORE ERROR {ai_query.prompt}: {e}', exc_info=True)
            return None
        except Exception as e:
            logger.error(f'Semantic cache ERROR lookup({ai_query.prompt}): {e}', exc_info=True)
            return None
    ...
    async def store_semantic_cache_entry(
        ...
        try:
            ...
            logger.error(f'Semantic cache VECTOR STORE ERROR {request.prompt}: {e}', exc_info=True)
        except Exception as e:
            logger.error(f'Semantic cache ERROR store({request.prompt}): {e}', exc_info=True)
