    ...
    async with backend_connector.session_scope() as scoped_session:
        ...
        if org is None:
            raise HTTPException(
                ...
                detail={
                    "message": f"Organization: {azure_tenant_id} is not registered"
                },
            )
        ...
        if user is None:
            raise HTTPException(
                ...
                detail={
                    "message": f"User: {azure_user_id} is not enrolled in organization: {org.id}"
                },
            )
    identity = UserIdentity(organization_id=org.id, user_id=user.id)
    return identity
