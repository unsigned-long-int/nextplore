...
from nextplore_orchestrator.services.query_orchestrator.query_executor import (
    QueryExecutor,
)
...
class SimpleLlmOrchestrator(LlmOrchestrator):
    ...
    async def run(
        ...
        collection = await self.vector_search.search(
    ...
    @staticmethod
    def neighbours_to_ranked(
        query: str, neighbours: list[VectorNeighbour]
    ) -> list[RankedVector]:
        return [
...
class ExpandedLlmOrchestrator(LlmOrchestrator):
    ...
    async def run(
        ...
        response = await self.query_executor.execute(
            orm_context=orm_context, user_identity=user_identity
        )
        response.trace = self._build_trace(llm_spec=llm_spec, pipeline=pipeline)
        return response

    def _build_trace(
        self, llm_spec: LlmSpec, pipeline: RagPipelineResult
    ) -> PipelineTrace:
        return PipelineTrace(
            ...
            vector_hits=[
                SubQuerySearchResult(
                    ...
                    vector_hits=[
                        VectorHit(
                            table=vn.orm_metadata.table_name,
                            score=vn.score,
                            snippet=vn.snippet,
                        )
            ...
            rrf_ranking=[
                RrfEntry(
                    table=rv.vector.orm_metadata.table_name,
                    rrf_score=rv.rrf_score,
                    rank=rv.rank,
                )
                for rv in pipeline.ranked
            ],
            schema_context=[
                entry.vector.orm_metadata.table_name
                for entry in pipeline.ranked[: self.rag_pipeline.top_n]
            ],
        )
