...
def _valid_kid(entry: CacheEntry, kid: str | None) -> bool:
    return not kid or kid in entry.kid_index


def _valid_jwk(entry: CacheEntry, kid: str | None) -> bool:
...
class JWKSFetcher:
    def __init__(self, jwks_cache: JWKSCacheService, ttl: int = 600):
        ...
        self._client = httpx.AsyncClient(
            timeout=5.0, limits=httpx.Limits(max_connections=10)
        )

    async def get_jwks(
        self, jwks_url: str, expected_kid: str | None = None
    ) -> dict[str, Any]:
        now = time.time()
        ...
        async with lock:
            ...
            try:
                resp = await self._client.get(jwks_url, headers=headers)
                ...
                if entry and _valid_kid(entry, expected_kid):
                    logger.warning(
                        f"Serving STALE JWKS for {jwks_url} due to fetch error"
                    )
                    return entry.jwks
                raise HTTPException(
