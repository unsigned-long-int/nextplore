...
@router.get('/organizations/register/verify', response_model=EmailVerificationStatusResponse)
@limiter.limit('5/minute;20/hour')
async def verify_email_token(
        request: Request,
        token: str,
        onboarding_service: OnboardingService = Depends(get_onboarding_service),
) -> Optional[EmailVerificationStatusResponse]:
    ...
    try:
        ...
        logger.error(
            f'Email verification failed with DB error: {str(e)}',
            exc_info=True
        )
        raise HTTPException(
        ...
        logger.error(
            f'Unexpected verification email error: {str(e)}.',
            exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={'message': f'Unexpected error while verifying email: {str(e)}'}
        )
