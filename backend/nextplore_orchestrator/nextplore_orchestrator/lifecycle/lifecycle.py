...
from nextplore_orchestrator.api.dependencies.authentication import (
    JWKSFetcher,
    TokenVerifier,
)
...
from nextplore_orchestrator.clients.factory import ClientsFactory, ClientsRegistry
...
async def lifespan(app: FastAPI):
    ...
    orchestrator_cache_client = BaseCache(
        namespace="nextplore_orchestrator", version="v1"
    )
    ...
    vector_searcher = VectorSearcher(
        embedding_client=registry.embedding_client, vector_client=registry.vector_client
    )
    ...
    app.state.onboarding_service = OnboardingService(
        db_connector=backend_connector, cache_service=orchestrator_cache_service
    )
