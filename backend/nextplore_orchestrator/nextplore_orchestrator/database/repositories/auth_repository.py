...
from nextplore_orchestrator.database.models import (
    OnboardingRequestORM,
    OrganizationORM,
    UserORM,
)
from nextplore_orchestrator.domain.mappers import (
    onboarding_request_from_orm,
    organization_from_orm,
)
from nextplore_orchestrator.domain.models import OnboardingRequest, Organization, User
...
class AuthRepository:
    ...
    async def get_org(self, azure_tenant_id: str) -> Organization | None:
        try:
            result = await self._session.execute(
                select(OrganizationORM).where(
                    OrganizationORM.azure_tenant_id == azure_tenant_id
                )
            )
    ...
    async def get_kek_kid(self, organization_id: UUID) -> str:
        try:
            result = await self._session.execute(
                select(OrganizationORM.kek_kid).where(
    ...
    async def get_onboarding_request_by_domain(
        self, email_domain: str
    ) -> OnboardingRequest | None:
        try:
            result = await self._session.execute(
                select(OnboardingRequestORM).where(
                    OnboardingRequestORM.domain == email_domain
                )
            )
    ...
    async def delete_onboarding_request(self, onboarding_id: UUID) -> None:
        try:
            stmt = delete(OnboardingRequestORM).where(
    ...
    async def get_onboarding_request_by_verification_token(
        ...
        try:
            result = await self._session.execute(
                ...
                .where(
                    OnboardingRequestORM.verification_token_expires_at
    ...
    async def verify_email(self, request_id: UUID) -> None:
        try:
            stmt = (
                ...
                .values(
                    {"email_verified": True, "verified_at": datetime.now(timezone.utc)}
                )
            )
            result = await self._session.execute(stmt)
            if result.rowcount == 0:
