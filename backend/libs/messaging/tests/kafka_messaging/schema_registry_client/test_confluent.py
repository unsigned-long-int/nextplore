...
from kafka_messaging.schema_registry_client.confluent import (
...
class TestSchemaRegistryClient(unittest.TestCase):
    def setUp(self):
        ...
        self.schema_dispatcher_mock.return_value = (
            '{"type":"record","name":"X","fields":[]}'
        )

    @patch(
        "kafka_messaging.schema_registry_client.confluent.SchemaRegistryClient",
        autospec=True,
    )
    @patch("kafka_messaging.schema_registry_client.confluent.Schema", autospec=True)
    def test_registers_when_latest_diff_registers_and_caches(
        self, schema_mock, sr_client_class_mock
    ):
        ...
        client = ConfluentSchemaRegistryClient(
            self.registry_url, self.schema_dispatcher_mock
        )
        schema_id = client.register(self.topic)

        self.schema_dispatcher_mock.assert_called_once_with(self.topic)
        schema_mock.assert_called_once_with(
            self.schema_dispatcher_mock(self.topic), "AVRO"
        )
        sr_client.register_schema.assert_called_once_with(
            self.subject, schema_mock.return_value
        )
    ...
    @patch("kafka_messaging.schema_registry_client.confluent.Schema", autospec=True)
    def test_registers_when_latest_same_uses_existing_id_no_register(
        self, schema_mock, sr_client_class_mock
    ):
        sr_client = sr_client_class_mock.return_value
    ...
    @patch("kafka_messaging.schema_registry_client.confluent.Schema", autospec=True)
    def test_registers_when_subject_not_found_registers(
        self, schema_mock, sr_client_class_mock
    ):
        sr_client = sr_client_class_mock.return_value
    ...
    @patch("kafka_messaging.schema_registry_client.confluent.Schema", autospec=True)
    def test_cache_hit_short_circuits_registry_calls(
        self, schema_mock, sr_client_class_mock
    ):
        sr_client = sr_client_class_mock.return_value
    ...
    def test_get_schema_str_by_id_delegates(self, sr_client_class_mock):
        sr_client = sr_client_class_mock.return_value
        sr_client.get_schema.return_value.schema_str = (
