...
class TestAvroCodec(unittest.TestCase):
    def setUp(self):
        ...
        self.schema_registry_client_mock.get_schema_str_by_id.return_value = (
            self.schema_str
        )
        self.schema_dispatcher_mock = MagicMock()
        self.schema_dispatcher_mock.return_value = self.schema_str
        self.avro_codec = AvroCodec(
            self.schema_registry_client_mock, self.schema_dispatcher_mock
        )
    ...
    @patch("kafka_messaging.codec.avro.schemaless_writer")
    def test_serialize_calls_to_avro_values_with_enriched_payload(
        self, schemaless_writer_mock, to_avro_values_mock
    ):
    ...
    def test_deserialize_reads_using_schema_from_registry(self, schemaless_reader_mock):
        ...
        self.schema_registry_client_mock.get_schema_str_by_id.assert_called_once_with(
            self.schema_id
        )
