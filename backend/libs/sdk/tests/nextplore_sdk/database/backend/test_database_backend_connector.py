...
class TestDatabaseBackendConnector(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('nextplore_sdk.database.backend.database_backend_connector.create_async_engine')
    @patch('nextplore_sdk.database.backend.database_backend_connector.async_sessionmaker')
    ...
    async def test_scoped_with_rls(
        ...
        org = uuid.uuid4()
        usr = uuid.uuid4()
        async with self.database_backend_connector.session_scope(org, usr) as session_scope:
            self.assertIs(session_scope, session_factory_mock)
            executed_sql = [str(call.args[0]) for call in session_factory_mock.execute.call_args_list]
