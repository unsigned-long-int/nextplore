...
class TestGcpPostgresqlPg8000Adapter(unittest.TestCase):
    def setUp(self):
        self.cred_path = "/Users/nik/Downloads/nextplore-470323-18ec66288496.json"
        self.adapter = GcpPostgresqlPg8000Adapter()

    @patch(
        "nextplore_sdk.database.connection_maker.driver_adapters.postgresql.gcp_pg8000_adapter.GcpCloudSqlConnector"
    )
    ...
    def test_connect_forwards_default(self, gcp_cloud_sql_connector_mock):
        ...
        connector_mock.connect.return_value = conn_mock
        gcp_cloud_sql_connector_mock.get.return_value = connector_mock
        conn = self.adapter.connect(host="localhost", database="mydb")
        gcp_cloud_sql_connector_mock.get.assert_called_once()
        connector_mock.connect.assert_called_once_with(
            "localhost",
            driver="pg8000",
