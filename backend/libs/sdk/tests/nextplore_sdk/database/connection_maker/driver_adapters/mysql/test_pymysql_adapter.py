...
class TestMysqlPyMysqlAdapter(unittest.TestCase):
    ...
    @patch(
        "nextplore_sdk.database.connection_maker.driver_adapters.mysql.pymysql_adapter.ssl"
    )
    @patch(
        "nextplore_sdk.database.connection_maker.driver_adapters.mysql.pymysql_adapter.pymysql"
    )
    def test_connect_happy_path(self, pymysql_mock, ssl_mock):
        ctx = MagicMock()
    ...
    def test_connect_forwards_defaults(self, pymysql_mock, ssl_mock):
        ctx = MagicMock()
        ssl_mock.create_default_context.return_value = ctx
        self.adapter.connect(
            host="localhost",
