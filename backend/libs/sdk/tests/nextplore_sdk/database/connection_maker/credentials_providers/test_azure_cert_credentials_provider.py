...
class TestAzureCertCredentialsProvider(unittest.TestCase):
    ...
    @patch(
        "nextplore_sdk.database.connection_maker.credentials_providers.azure_cert_credentials_provider.CertificateCredential"
    )
    def test_retrieves_cert_with_default_scope(self, certificate_credential_mock):
        cred_mock = MagicMock()
        ...
        cred_mock.get_token.assert_called_once_with(
            self.credentials_provider.DEFAULT_SCOPE
        )
    ...
    def test_retrieves_cert_with_override_scope(self, certificate_credential_mock):
        cred_mock = MagicMock()
        token_mock = MagicMock()
        token_mock.token = "test-token"
        cred_mock.get_token.return_value = token_mock
    ...
    @patch(
        "nextplore_sdk.database.connection_maker.credentials_providers.azure_cert_credentials_provider.SecretClient"
    )
    def test_byte_encodes_pem(self, secret_client_cls_mock):
        ...
        self.assertEqual(certs, b"-----BEGIN CERTIFICATE----- \nMIID...")

    @patch(
        "nextplore_sdk.database.connection_maker.credentials_providers.azure_cert_credentials_provider.SecretClient"
    )
    def test_byte_encodes_base64(self, secret_client_cls_mock):
        secret_client_instance = MagicMock()
        secret_mock = MagicMock()
