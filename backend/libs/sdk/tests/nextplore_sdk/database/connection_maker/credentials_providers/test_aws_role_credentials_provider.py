...
class TestAWSRoleCredentialsProvider(unittest.TestCase):
    ...
    @patch.dict(os.environ, {'AWS_ROLE_ARN': 'arn:aws:iam::123456789012:role/BootstrapRole'}, clear=False)
    @patch('uuid.uuid4')
    @patch('boto3.Session')
    def test_assumes_role(
        self,
        session_cls_mock,
        uuid4_mock
    ):
    ...
    def test_creds_composes_assume_and_token(self):
        with patch.object(self.provider, '_assume', return_value={'AccessKeyId': 'X', 'SecretAccessKey': 'Y', 'SessionToken': 'Z'}) as assume_mock, \
             patch.object(self.provider, '_token', return_value='final-token') as token_mock:
            result = self.provider.creds()

        self.assertEqual(result, 'final-token')
        assume_mock.assert_called_once_with()
        token_mock.assert_called_once_with({'AccessKeyId': 'X', 'SecretAccessKey': 'Y', 'SessionToken': 'Z'})
