...
from nextplore_sdk.database.connection_maker.engine.engine_build import (
    build_engine,
    invoke_engine,
)
...
class TestBuildEngine(unittest.IsolatedAsyncioTestCase):
    ...
    @patch(
        "nextplore_sdk.database.connection_maker.engine.engine_build.asyncio.to_thread",
        new_callable=AsyncMock,
    )
    @patch(
        "nextplore_sdk.database.connection_maker.engine.engine_build.STRATEGY_REGISTRY"
    )
    async def test_build_happy_path(self, strategy_registry_mock, to_thread_mock):
        ...
        self.strategy_instance.make_creator.assert_called_once_with(
            adapter=self.adapter_instance, creds_provider=self.creds_instance
        )
    ...
    async def test_build_raises_if_missing_strategy(
        self, strategy_registry_mock, to_thread_mock
    ):
        engine_mock = MagicMock()
        to_thread_mock.return_value = engine_mock
    ...
    async def test_build_not_sets_creds_provider_kwargs(
        self, strategy_registry_mock, to_thread_mock
    ):
        engine_mock = MagicMock()
        to_thread_mock.return_value = engine_mock
        ...
        strategies = {
            ("cloud", "db", "auth"): (self.strategy_mock, self.adapter_mock, None)
        }
        strategy_registry_mock.__getitem__.side_effect = lambda k: strategies[k]
        _ = await build_engine(self.profile_mock)
        self.strategy_mock.assert_called_once_with(self.profile_mock)
