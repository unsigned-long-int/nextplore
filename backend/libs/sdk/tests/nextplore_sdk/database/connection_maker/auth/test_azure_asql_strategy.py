...
class TestAzureIamAsqlStrategy(unittest.TestCase):
    ...
    @patch('nextplore_sdk.database.connection_maker.auth.azure_asql_strategy.make_token_bytes')
    @patch('nextplore_sdk.database.connection_maker.auth.azure_asql_strategy.resolve_ca_bundle')
    def test_makes_creator_with_creds_provider(
        self,
        resolve_ca_bundle_mock,
        make_token_bytes_mock
    ):
        ...
        driver_adapter_mock.connect.assert_called_once_with(
            ...
            attrs_before={
                self.strategy.ACCESS_TOKEN_ATTR: b''
            }
        )
