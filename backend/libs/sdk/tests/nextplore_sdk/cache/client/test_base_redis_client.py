...
class TestBaseCache(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sets_and_build_base_cache(
        self,
        get_redis_client_mock
    ):
    ...
    def test_build_key_from_mixed_parts(self):
        ...
        self.assertEqual(key, f'test-namespace:test-version:1:2:3:{str(uid_part)}')
    ...
    async def test_deletes_invalid_model(self):
        ...
        self.redis_client.get.return_value = InvalidModel(whatever='whatever').model_dump_json()
        base_cache = BaseCache(
            namespace=namespace,
            version=version,
            redis=self.redis_client
    ...
    async def test_gets_many_models(self):
        ...
        self.redis_client.get.return_value = json.dumps(dummy_models, default=pydantic_encoder)
        base_cache = BaseCache(
            namespace=namespace,
            version=version,
            redis=self.redis_client
    ...
    async def test_delete_by_prefix_deletes_only_matching(self):
        version = 'test-version'
        namespace = 'test-namespace'
        base_cache = BaseCache(namespace=namespace, version=version, redis=self.redis_client)
        ...
        self.redis_client.scan_iter = unittest.mock.MagicMock(side_effect=scan_iter_mock)
        ...
        self.redis_client.scan_iter.assert_called_once_with(match=expected_pattern, count=100)
