...
from nextplore_sdk.encryptor.cert.cert_generator import (
    Cert,
    CertGenerator,
    Encoding,
    x509,
...
class TestCertGenerator(unittest.TestCase):
    ...
    def _arrange_success(self, *, not_before):
        ...
        self.client.get_certificate.return_value = type(
            "CertProps", (), {"key_id": "kid-123"}
        )()
    ...
    def test_create_cert_success(self):
        ...
        self.mock_x509_loader.return_value.public_bytes.assert_called_once_with(
            Encoding.PEM
        )
    ...
    def test_azure_error_is_wrapped_and_logged(self):
        ...
        with patch(
            "nextplore_sdk.encryptor.cert.cert_generator.AzureError", new=FakeAzureError
        ):
