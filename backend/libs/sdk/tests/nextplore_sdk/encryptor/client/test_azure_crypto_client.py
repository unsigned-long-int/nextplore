...
class TestAzureCryptoClient(unittest.TestCase):
    ...
    def test_encrypt_secret_structure_and_values(self):
        ...
        self.mock_crypto_client.wrap_key.return_value = SimpleNamespace(encrypted_key=b'WRAPPED')

        plaintext = 's3cr3t'
        aad = {'tenant': 't1', 'kid': '123'}
        aad_bytes = json.dumps(aad, separators=(',', ':')).encode()

        expected_full = AESGCM(self._dek).encrypt(self._nonce, plaintext.encode(), aad_bytes)
