...
class TestAzureVaultKeyProvider(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_key_client_instance.create_rsa_key.return_value = (
            self.mock_created_key
        )
    ...
    def test_creates_vault(self):
        ...
        self.mock_key_client_cls.assert_called_once_with(
            vault_url="test-url", credential=self.mock_cred
        )
        self.mock_key_client_instance.create_rsa_key.assert_called_once_with(
            name=f"kek-{tenant}", size=3072
        )
