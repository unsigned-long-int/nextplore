...
class CertGenerator:
    def __init__(
        self,
        cert_name: str,
        client: CertificateClient | None = None,
    ...
    def create_cert(
        self, key_size: int | None = None, validity_in_months: int | None = None
    ) -> Cert:
        ...
        try:
            poller = self.cert_client.begin_create_certificate(
                certificate_name=self.cert_name, policy=policy, enabled=True
            )
            cert = poller.result()
            ...
            der = cert.cer
            public_pem = (
                x509.load_der_x509_certificate(der).public_bytes(Encoding.PEM).decode()
            )
            ...
            logger.error(
                "Certificate creation failed in Azure with error: %s", e, exc_info=True
            )
            raise AzureCertCreationFailed(
                f"Cert creation failed for: {self.cert_name} with error: {e}"
            ) from e
